...
public class NachosProcess implements Runnable {
	...
	*/
	private static Integer INIT_PID = 1000;
	private Integer mPID;

	//public static HashMap<Integer, Integer> Child_Parent_Map = new HashMap<>();
	...
	public void run() {
		...
		//System.out.println(JNachos.getCurrentProcess().getName()+" real running");
		myFunc.call(myArg);
	...
	public void kill() {
		...
		if (mSpace != null && !booted) {
			for(int i=0; i<mSpace.mNumPages; i++){
				// When a process terminates, its address space should be cleaned up (reclaim pages in RAM).
				if(mSpace.mPageTable[i].physicalPage != -1){
					// clean up RAM
					Arrays.fill(Machine.mMainMemory, mSpace.mPageTable[i].physicalPage* Machine.PageSize,
							(mSpace.mPageTable[i].physicalPage + 1) * Machine.PageSize, (byte) 0);
					// reset free bit map
					mSpace.mFreeMap.clear(mSpace.mPageTable[i].physicalPage);
					// clean up LRU_Buffer
					mSpace.LRU_BUFFER[mSpace.mPageTable[i].physicalPage] = null;
				}
				// clean up LRU_Stack
				if(mSpace.LRU_Stack.contains(mSpace.mPageTable[i])){
					mSpace.LRU_Stack.remove(mSpace.LRU_Stack.indexOf(mSpace.mPageTable[i]));
				}
				mSpace.mPageTable[i].physicalPage = -1;
				// clean up SWAP space
				JNachos.mSwapFree.clear(mSpace.mPageTable[i].swapPage);
				byte[] Zero = new byte[Machine.PageSize];
				JNachos.swap.writeAt(Zero, Machine.PageSize, mSpace.mPageTable[i].swapPage*Machine.PageSize);
			}
			// reset LRU pointer
			mSpace.pointer = mSpace.mFreeMap.find();
			mSpace.mFreeMap.clear(mSpace.pointer);
		}
		mSpace = null;
	}
