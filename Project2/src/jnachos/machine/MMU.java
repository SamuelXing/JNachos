...
public class MMU {
	...
	public static ExceptionType translate(int virtAddr, int[] physAddr, int size, boolean writing) {
		...
		if(JNachos.getCurrentProcess().getSpace().LRU_Stack.contains(MMU.mPageTable[vpn])){
			JNachos.getCurrentProcess().getSpace().LRU_Stack.remove(
					JNachos.getCurrentProcess().getSpace().LRU_Stack.indexOf(MMU.mPageTable[vpn])
			);
		}
		JNachos.getCurrentProcess().getSpace().LRU_Stack.add(MMU.mPageTable[vpn]);
		...
		if (mTlb == null) { // => page table => vpn is index into table
			if (vpn >= mPageTableSize) {
				...
				Debug.print('a', "virtual page "+ vpn +" # " + virtAddr + "  too large for page table size " + mPageTableSize);
